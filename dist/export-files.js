...
export function context_export_dir() {
    return join(dirname(default_context_db_path()), EXPORT_DIR_NAME);
}
export function default_context_export_path(source_id, chunk_id) {
    const suffix = chunk_id ? `-${safe_path_segment(chunk_id)}` : '';
    return join(context_export_dir(), `${safe_path_segment(source_id)}${suffix}.txt`);
}
export function resolve_context_export_path(file_path, cwd, source_id, chunk_id) {
    if (!file_path)
        return default_context_export_path(source_id, chunk_id);
    return isAbsolute(file_path) ? file_path : resolve(cwd, file_path);
}
export function write_context_export_file(file_path, content) {
...
export function cleanup_context_exports(policy = parse_context_retention_policy(), now = Date.now()) {
...
function safe_path_segment(value) {
    return (value.replace(/[^a-zA-Z0-9._-]+/g, '_').slice(0, 120) || 'context');
}
//# sourceMappingURL=export-files.js.map