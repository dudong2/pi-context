...
const ContextStoreClass = class ContextStore {
	...
	store(input: StoreContextInput): StoredContextOutput | null {
		...
		const content_hash = createHash('sha256')
			...
			.digest('hex');
		const session_id =
			redact_metadata(input.session_id ?? this.session_id) ?? null;
		const project_path =
			redact_metadata(input.project_path ?? this.project_path) ?? null;
		...
		const stored = with_sqlite_transaction(
			this.db,
			() => {
				insert.run(
					source_id,
	...
	record_returned_bytes(source_ids: string[], returned_bytes: number): void {
		...
		if (unique_ids.length === 0 || returned_bytes <= 0) return;
...
export const ContextStore: ContextStoreConstructor = ContextStoreClass;
