...
} satisfies Record<ContextSettingsPreset, ContextSettingsPresetDefinition>;
...
export function load_context_settings_config(): ContextSettingsConfig | null {
	...
	try {
		const parsed = (existsSync(path)
			...
			: null) as Partial<ContextSettingsConfig> | null;
...
export function normalize_context_settings_config(
	...
	return {
		...
		purge_on_shutdown:
			value.purge_on_shutdown === true ||
			value.purge_on_shutdown === false
