...
function masked(match: string, name: string): string {
...
function redact_assignments(text: string): RedactionResult {
	let count = 0;
	let redacted = text.replace(
		SECRET_ASSIGNMENT,
		(match, assignment: string, value: string) => {
...
function is_token_char(code: number): boolean {
	return (
		(code >= 48 && code <= 57) ||
		(code >= 65 && code <= 90) ||
		(code >= 97 && code <= 122) ||
...
function token_end(text: string, start: number): number {
	for (let index = start; index < text.length; index += 1) {
		if (!is_token_char(text.charCodeAt(index))) return index;
	}
	return text.length;
}

/** Redact two long token segments separated by a dot in one forward scan. */
function redact_dotted_tokens(text: string): RedactionResult {
	const pieces: string[] = [];
	let cursor = 0;
	let index = 0;
	let count = 0;
	for (; index < text.length; ) {
		...
		if (index - first_start < 40 || text.charCodeAt(index) !== 46) continue;
		const second_start = index + 1;
		const end = token_end(text, second_start);
		if (end - second_start < 40) continue;
	...
	if (count === 0) return { redacted: text, count: 0 };
	pieces.push(text.slice(cursor));
	return { redacted: pieces.join(''), count };
}

export function redact_metadata(value: string): string;
export function redact_metadata(value: null): null;
export function redact_metadata(value: undefined): undefined;
export function redact_metadata(
