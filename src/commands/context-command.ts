...
function show_context_stats(
	...
	ctx.ui.notify(
		format_stats(stats, { audience: 'tui', title: false }),
		'info',
	);
}

function show_context_settings(ctx: ExtensionCommandContext): void {
	const scope = scope_from_context(ctx);
	const stats = get_context_store(scope).stats(scope);
	ctx.ui.notify(
		format_context_settings_status(stats, { audience: 'tui' }),
		'info',
	);
}

function purge_context(
	ctx: ExtensionCommandContext,
	options: { expired?: boolean; source_id?: string; older_than_days?: number },
): void {
	const scope = scope_from_context(ctx);
	const store = get_context_store(scope);
	if (options.expired) {
		ctx.ui.notify(
			format_purge_details({ deleted: store.cleanup().deleted }),
			'info',
		);
		return;
	}
...
export function register_context_commands(pi: ExtensionAPI): void {
	pi.registerCommand('context', {
		...
		getArgumentCompletions: (prefix) =>
			['list', 'stats', 'settings', 'purge'].flatMap((item) =>
				item.startsWith(prefix.trim())
			...
			switch (sub || 'list') {
				...
				case 'settings': {
					...
					if (!is_context_settings_preset(preset)) {
						ctx.ui.notify(
							'Usage: /context settings <default|light|balanced|research|archive>',
							'warning',
						);
						return;
					}
